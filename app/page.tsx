...
function buildCommunications(rows: string[][], customers: Customer[], aircraft: Aircraft[], templates: Template[], airports: Airport[] = [], fboSelections: Record<string, number | null> = {}, contacts: Contact[] = []) {
  const errors: string[] = []
  const emails: Email[] = []
  ...
  const customerEmails = customer ? Array.from(new Set(contacts.filter(c => c.customerId === customer.id).flatMap(c => c.emails).map(e => e.trim().toLowerCase()).filter(Boolean))) : []
  ...
  const locations=new Map<string,string[]>(); for(const r of rows) { const departure=String(r[4]||'').trim().toUpperCase(); const arrival=String(r[6]||'').trim().toUpperCase(); if(departure&&!locations.has(departure))locations.set(departure,r); if(arrival&&!locations.has(arrival))locations.set(arrival,r) }
  let id=20000
...
function ScheduleProcessor({ schedule, setSchedule, notify, deals, customers, aircraft, templates, contacts, setGeneratedEmails, setPage }: { schedule: string; setSchedule: (s: string) => void; notify: (s: string) => void; deals: Deal[]; customers: Customer[]; aircraft: Aircraft[]; templates: Template[]; contacts: Contact[]; setGeneratedEmails: (e: Email[]) => void; setPage: (p: Page) => void }) {
  const sample='27183\t4/04\t19:00\tN838C\tKRUQ\tSALISBURY, NC\tKJFK\tNEW YORK, NY\t75\tGP  / CM\t4/04\t20:27\t1\tAhold\tAndres\tSalisbury Corporate Hangar\n27183\t4/04\t22:35\tN838C\tKJFK\tNEW YORK, NY\tKOPF\tMIAMI\t86\tGP  / CM\t4/05\t00:13\t2\tAhold\tAndres\tMODERN AVIATION\n27183\t4/05\t10:35\tN838C\tKOPF\tMIAMI\tKRUQ\tSALISBURY, NC\t86\tGP  / CM\t4/05\t00:13\t2\tAhold\tAndres\tFONTAINEBLEAU AVIATION'
  ...
  const locations=useMemo(()=>{const m=new Map<string,string[]>();for(const r of rows)for(const raw of [r[4],r[6]]){const c=String(raw||'').trim().toUpperCase();if(c&&!m.has(c))m.set(c,r)}return Array.from(m.entries()).map(([icao,row])=>({icao,row}))},[rows]); const fboTemplates=templates.filter(t=>t.type==='FBO')
  const options=(icao:string)=>{const selected=fboSelections[icao];const base=fboTemplates;return selected&&!base.some(t=>t.id===selected)?[...base,...fboTemplates.filter(t=>t.id===selected)]:base}
  const result=useMemo(()=>buildCommunications(rows,customers,aircraft,templates,airportData,fboSelections,contacts),[rows,customers,aircraft,templates,airportData,JSON.stringify(fboSelections),contacts]); const errors=result.errors; const communications=result.emails.filter(e=>e.type==='Customer'||!removed.has(e.icao)); const visible=locations.filter(x=>!removed.has(x.icao))
  ...
  return <div className="panel processor"><div className="steps"><span className="done">1 <b>Paste Schedule</b></span><span className={step===2?'done':''}>2 <b>Build Emails</b></span></div>
    ...
    {step===2&&<><div className="reviewSummary"><div><b>Customer</b><span>{customer?.name||'Not found'}</span></div><div><b>Customer Emails</b><span>1</span></div><div><b>FBO Locations</b><span>{visible.length}</span></div><div><b>FBOs Selected</b><span>{visible.filter(x=>fboSelections[x.icao]).length}</span></div></div><div className="processorDeals"><div className="processorDealsHead"><div><b>Customer Deals</b><span>{customer?.name||'Not found'}</span></div><small>Assigned pricing programs for this customer</small></div><div className="processorDealGrid">{customer?.dealIds?.length ? customer.dealIds.map(id=>deals.find(d=>d.id===id)).filter(Boolean).map(d=><div className="dealCard processorDealCard" key={d!.id} style={{borderTopColor:d!.color}}><div className="dealBadge" style={{background:`${d!.color}18`,color:d!.color,border:`1px solid ${d!.color}40`}}>{d!.provider.slice(0,2).toUpperCase()}</div><div><h3>{d!.name}</h3><b style={{color:d!.color}}>{d!.provider}</b><p>{d!.description}</p><div className="dealMeta"><span style={{color:d!.tankering?'#0b9c63':'#8a97a7'}}>{d!.tankering?'Tankering enabled':'Tankering not included'}</span></div></div></div>) : <div className="processorNoDeals">No deals assigned to this customer.</div>}</div></div>{errors.length>0&&<div className="validationError"><AlertTriangle size={18}/><div><b>Setup required</b>{errors.map((e,i)=><p key={i}>{e}</p>)}</div></div>}
...
function Customers({ customers, setCustomers, deals, setDeals, contacts, setContacts, notify }: { customers: Customer[]; setCustomers: (c: Customer[]) => void; deals: Deal[]; setDeals: (d: Deal[]) => void; contacts: Contact[]; setContacts: (c: Contact[]) => void; notify: (s: string) => void }) {
  const [activeTab, setActiveTab] = useState<'Customers' | 'Contacts' | 'Deals'>('Customers')
  ...
  const [rule, setRule] = useState('Customer + FBO emails')
  const [includeTripNumber, setIncludeTripNumber] = useState(false)
  const [homeBase, setHomeBase] = useState('')
  const [contactName, setContactName] = useState('')
  const [contactCustomerId, setContactCustomerId] = useState<number | null>(null)
  const [contactFields, setContactFields] = useState<{name:string;email:string}[]>([{name:'',email:''}])
  ...
  const [dealColor, setDealColor] = useState('#1976e5')
  const [dealTankering, setDealTankering] = useState(false)
  function openCustomer(c?: Customer) { setEditing(c||null); setName(c?.name||''); setDealIds(c?.dealIds||[]); setRule(c?.rule||'Customer + FBO emails'); setIncludeTripNumber(c?.includeTripNumber??false); setHomeBase(c?.homeBase||''); setEditorOpen(true) }
  function openContact(c?: Contact) { setContactEditing(c||null); setContactName(c?.name||''); setContactCustomerId(c?.customerId??(customers[0]?.id||null)); setContactFields(c?.emails?.length ? c.emails.map(email=>({name:c.name,email})) : [{name:'',email:''}]); setContactEditorOpen(true) }
  function closeEditors() { setEditorOpen(false); setContactEditorOpen(false); setEditing(null); setContactEditing(null); setName(''); setDealIds([]); setRule('Customer + FBO emails'); setIncludeTripNumber(false); setHomeBase(''); setContactName(''); setContactCustomerId(customers[0]?.id||null); setContactFields([{name:'',email:''}]) }
  function saveCustomer() { if(!name.trim()) return notify('Customer name is required'); const normalizedHomeBase=homeBase.trim().toUpperCase(); if(!/^[A-Z]{4}$/.test(normalizedHomeBase)) return notify('Home base location must be a 4-letter ICAO code'); const item:Customer={id:editing?.id||Date.now(),name:name.trim(),dealIds,rule,includeTripNumber,homeBase:normalizedHomeBase}; setCustomers(editing?customers.map(c=>c.id===editing.id?item:c):[...customers,item]); notify(editing?'Customer updated':'Customer created'); closeEditors() }
  function saveContact() {
    if(!contactCustomerId) return notify('Select a customer for these contacts')
    const fields=contactFields.map(field=>({name:field.name.trim(),email:field.email.trim().toLowerCase()}))
    if(fields.some(field=>!field.name)) return notify('Contact name is required for every email')
    if(fields.some(field=>!field.email)) return notify('Email address is required for every contact')
    if(fields.some(field=>!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(field.email))) return notify('Enter valid email addresses')
    const items:Contact[]=fields.map((field,index)=>({id:contactEditing&&index===0?contactEditing.id:Date.now()+index,customerId:contactCustomerId,name:field.name,emails:[field.email]}))
    setContacts(contactEditing?[...contacts.filter(c=>c.id!==contactEditing.id),...items]:[...contacts,...items])
    notify(contactEditing?'Contact updated':'Contact created')
    closeEditors()
  }
  function addContactField() { setContactFields(fields=>[...fields,{name:'',email:''}]) }
  function removeContactField(index:number) { setContactFields(fields=>fields.length===1?[{name:'',email:''}]:fields.filter((_,i)=>i!==index)) }
  function openDeal(d?: Deal) { setDealEditorOpen(true); setDealEditing(d || null); setDealName(d?.name || ''); setDealProvider(d?.provider || ''); setDealDescription(d?.description || ''); setDealColor(d?.color || '#1976e5'); setDealTankering(d?.tankering || false) }
  function closeDeal() { setDealEditing(null); setDealEditorOpen(false); setDealName(''); setDealProvider(''); setDealDescription(''); setDealColor('#1976e5'); setDealTankering(false) }
  function saveDeal() { if (!dealName.trim() || !dealProvider.trim()) return notify('Deal name and provider are required'); const item:Deal={id:dealEditing?.id||Date.now(),name:dealName.trim(),provider:dealProvider.trim(),description:dealDescription.trim(),color:dealColor,tankering:dealTankering}; setDeals(dealEditing?deals.map(d=>d.id===dealEditing.id?item:d):[...deals,item]); notify(dealEditing?'Deal updated':'Deal created'); closeDeal() }
  function removeDeal(id:number) { setDeals(deals.filter(d=>d.id!==id)); notify('Deal deleted') }
  return <div className="panel full">
    <div className="sectionIntro"><div><h3>{activeTab}</h3><p>{activeTab==='Customers'?'Manage customers, pricing programs and communication rules.':activeTab==='Contacts'?'Manage customer contacts and all email addresses used for customer drafts.':'Create and manage the pricing programs assigned to customers.'}</p></div>{activeTab==='Deals'?<button className="primary" onClick={()=>openDeal()}><Plus size={16}/> Add Deal</button>:<button className="primary" onClick={()=>activeTab==='Customers'?openCustomer():openContact()}><Plus size={16}/>{activeTab==='Customers'?'Add Customer':'Add Contact'}</button>}</div>
    <div className="customerTabs"><button className={activeTab==='Customers'?'customerTab active':'customerTab'} onClick={()=>setActiveTab('Customers')}>Customers</button><button className={activeTab==='Contacts'?'customerTab active':'customerTab'} onClick={()=>setActiveTab('Contacts')}>Contacts</button><button className={activeTab==='Deals'?'customerTab active':'customerTab'} onClick={()=>setActiveTab('Deals')}>Deals</button></div>
    {activeTab==='Customers'&&<div className="tableWrap"><table><thead><tr><th>Customer Name</th><th>Customer Deals</th><th>Trip Number</th><th>Home Base Location</th><th>Actions</th></tr></thead><tbody>{customers.map(c=><tr key={c.id}><td><b>{c.name}</b></td><td>{deals.filter(d=>c.dealIds.includes(d.id)).map(d=><span className="pill" key={d.id} style={{background:`${d.color}18`,color:d.color,border:`1px solid ${d.color}35`}}>{d.provider}</span>)}{c.dealIds.length===0&&<span className="mutedText">No deals assigned</span>}</td><td><span className={c.includeTripNumber?'yes':'no'}>{c.includeTripNumber?'Included':'Not included'}</span></td><td><b>{c.homeBase||'—'}</b></td><td><button className="tiny" onClick={()=>openCustomer(c)}><Edit3 size={13}/></button></td></tr>)}</tbody></table></div>}
    {activeTab==='Contacts'&&<div className="tableWrap"><table><thead><tr><th>Contact Name</th><th>Customer</th><th>Email Addresses</th><th>Actions</th></tr></thead><tbody>{contacts.length?contacts.map(c=><tr key={c.id}><td><b>{c.name}</b></td><td>{customers.find(customer=>customer.id===c.customerId)?.name||'Unknown customer'}</td><td>{c.emails.map(email=><a key={email} className="pill contactEmailPill" href={`mailto:${email}`}>{email}</a>)}</td><td><button className="tiny" onClick={()=>openContact(c)}><Edit3 size={13}/></button><button className="tiny" onClick={()=>{setContacts(contacts.filter(x=>x.id!==c.id));notify('Contact removed')}}><Trash2 size={13}/></button></td></tr>):<tr><td colSpan={4} style={{textAlign:'center',padding:28}}>No contacts have been added yet.</td></tr>}</tbody></table></div>}
    {activeTab==='Deals'&&<div className="dealGrid">{deals.map(d=><div className="dealCard" key={d.id} style={{borderTopColor:d.color}}><div className="dealBadge" style={{background:`${d.color}18`,color:d.color,border:`1px solid ${d.color}40`}}>{d.provider.slice(0,2).toUpperCase()}</div><div><h3>{d.name}</h3><b style={{color:d.color}}>{d.provider}</b><p>{d.description}</p><div className="dealMeta"><span style={{color:d.tankering?'#0b9c63':'#8a97a7'}}>{d.tankering?'Tankering enabled':'Tankering not included'}</span></div></div><div className="cardActions"><button onClick={()=>openDeal(d)}><Edit3 size={14}/> Edit</button><button onClick={()=>removeDeal(d.id)}><Trash2 size={14}/> Delete</button></div></div>)}</div>}
    {dealEditorOpen&&<div className="editor"><h3>{dealEditing?'Edit Customer Deal':'New Customer Deal'}</h3><div className="formGrid"><label>Deal name<input value={dealName} onChange={e=>setDealName(e.target.value)} placeholder="e.g. CAA Network Pricing"/></label><label>Provider / program<input value={dealProvider} onChange={e=>setDealProvider(e.target.value)} placeholder="e.g. CAA"/></label><label>Deal color<div className="colorPicker"><input type="color" value={dealColor} onChange={e=>setDealColor(e.target.value)}/><span style={{background:dealColor}}></span><b>{dealColor.toUpperCase()}</b></div></label><label className="checkField"><input type="checkbox" checked={dealTankering} onChange={e=>setDealTankering(e.target.checked)}/> Tankering included in this deal</label><label className="wide">Description<textarea value={dealDescription} onChange={e=>setDealDescription(e.target.value)} placeholder="Describe what this deal provides..."/></label></div><div className="formActions"><button className="secondary" onClick={closeDeal}>Cancel</button><button className="primary" onClick={saveDeal}><Check size={15}/> Save Deal</button></div></div>}
    {editorOpen&&<div className="editor"><h3>{editing?'Edit Customer':'New Customer'}</h3><div className="formGrid"><label>Customer name<input value={name} onChange={e=>setName(e.target.value)} placeholder="Customer name"/></label><label>Communication rules<select value={rule} onChange={e=>setRule(e.target.value)}><option>Customer + FBO emails</option><option>Customer email only</option><option>Full fuel communication</option><option>Custom template</option></select></label><label>Home base location<input value={homeBase} maxLength={4} onChange={e=>setHomeBase(e.target.value.replace(/[^a-zA-Z]/g,'').slice(0,4).toUpperCase())} placeholder="KTEB"/><small>4-letter ICAO airport identifier</small></label><label className="checkField"><input type="checkbox" checked={includeTripNumber} onChange={e=>setIncludeTripNumber(e.target.checked)}/> Include Trip Number in customer emails</label><div className="wide"><b className="labelTitle">Customer Deals</b><p className="fieldHint">Select every pricing program this customer has.</p><div className="dealChecks">{deals.map(d=><label key={d.id} style={{borderLeftColor:d.color}}><input type="checkbox" checked={dealIds.includes(d.id)} onChange={e=>setDealIds(e.target.checked?[...dealIds,d.id]:dealIds.filter(id=>id!==d.id))}/><span style={{color:d.color}}>{d.name}</span><small>{d.provider}</small></label>)}</div></div></div><div className="formActions"><button className="secondary" onClick={closeEditors}>Cancel</button><button className="primary" onClick={saveCustomer}><Check size={15}/>Save Customer</button></div></div>}
    {contactEditorOpen&&<div className="editor"><h3>{contactEditing?'Edit Contact':'New Contact'}</h3><div className="formGrid"><label className="wide">Customer<select value={contactCustomerId||''} onChange={e=>setContactCustomerId(e.target.value?Number(e.target.value):null)}>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><small>Select the customer once. Each row below is a separate contact and email address.</small></label><div className="wide"><span className="labelTitle">Contacts and email addresses</span><div className="contactEmailFields">{contactFields.map((field,index)=><div className="contactEmailRow contactPersonRow" key={index}><input value={field.name} onChange={e=>setContactFields(values=>values.map((v,i)=>i===index?{...v,name:e.target.value}:v))} placeholder="Contact name"/><input type="email" value={field.email} onChange={e=>setContactFields(values=>values.map((v,i)=>i===index?{...v,email:e.target.value}:v))} placeholder="name@company.com"/>{index===contactFields.length-1?<button type="button" className="addEmailButton" title="Add another contact and email" onClick={addContactField}><Plus size={15}/></button>:<button type="button" className="removeEmailButton" title="Remove contact" onClick={()=>removeContactField(index)}><X size={14}/></button>}</div>)}</div><small className="fieldHint">Use the blue + button to add another contact name and email address.</small></div></div><div className="formActions"><button className="secondary" onClick={closeEditors}>Cancel</button><button className="primary" onClick={saveContact}><Check size={15}/>Save Contact</button></div></div>}
  </div>
}
function Toolbar({ placeholder, value, onChange }: { placeholder: string; value?: string; onChange?: (value:string)=>void }) { return <div className="toolbar"><div><Search size={14} /><input value={value ?? ''} onChange={e=>onChange?.(e.target.value)} placeholder={placeholder} /></div></div> }
...
function AircraftPage({ aircraft, setAircraft, customers, notify }: { aircraft: Aircraft[]; setAircraft: (a: Aircraft[]) => void; customers: Customer[]; deals?: Deal[]; notify: (s: string) => void }) {
  ...
  function normalizeExpiration(value: string) {
    ...
    const match = raw.match(/^(0[1-9]|1[0-2])\/(\d{2}|20\d{2})$/)
    if (!match) return raw
    const year = match[2].length === 2 ? `20${match[2]}` : match[2]
    return `${match[1]}/${year}`
  }
  function formatExpirationDisplay(value: string) {
    const match = value.trim().match(/^(0[1-9]|1[0-2])\/(\d{2}|20\d{2})$/)
    if (!match) return value
    const year = match[2].length === 4 ? match[2].slice(-2) : match[2]
    return `${match[1]}/${year}`
  }
  function open(a?: Aircraft) { setEditorOpen(true); setEditing(a || null); setTail(a?.tail || ''); setType(a?.type || ''); setCustomerId(a?.customerId ?? null); setHomeBase(a?.homeBase || ''); setAvcard((a?.avcard || '').replace(/\D/g,'')); setExpiration(normalizeExpiration(a?.expiration || '')) }
  ...
  function save() { const rawCard = avcard.trim(); const cardDigits = rawCard.replace(/\D/g, ''); if (!tail.trim() || !type.trim()) return notify('Tail number and aircraft type are required'); if (rawCard && cardDigits.length !== 16) return notify('AVCARD must contain exactly 16 digits'); if (expiration && !/^((0[1-9])|(1[0-2]))\/(20\d{2})$/.test(expiration)) return notify('Expiration must be MM/YYYY'); const item: Aircraft = { id: editing?.id || Date.now(), tail: tail.trim().toUpperCase(), type: type.trim(), customerId, homeBase: homeBase.trim().toUpperCase(), avcard: cardDigits, expiration: expiration.trim(), active: editing?.active ?? true }; if (editing) setAircraft(aircraft.map(a => a.id === editing.id ? item : a)); else setAircraft([...aircraft, item]); notify(editing ? 'Aircraft updated' : 'Aircraft added'); close() }
...
function RichEditor({ value, onChange, multiline, placeholder, allowTable, notify }: { value: string; onChange: (v: string) => void; multiline?: boolean; placeholder?: string; allowTable?: boolean; notify: (s: string) => void }) {
  ...
  function changeHighlight(color: string) { if (color) command('hiliteColor', color) }
  function changeSize(next: number) { const clamped=Math.max(1,Math.min(7,next)); setFontSize(clamped); command('fontSize',String(clamped)) }
  return <div className="richEditor"><div className="richToolbar">
    {multiline && <button type="button" className="toolbarIconButton" title="Bold" onMouseDown={e=>e.preventDefault()} onClick={()=>command('bold')}><b>B</b></button>}
    {multiline && <label className="colorTool" title="Text color"><span className="colorDot"/><input type="color" defaultValue="#111827" aria-label="Text color" onChange={e=>changeColor(e.target.value)}/></label>}
    {multiline && <div className="highlightTool"><button type="button" className="highlightButton" title="Highlight color" aria-label="Highlight color" onMouseDown={e=>e.preventDefault()} onClick={()=>setHighlightOpen(v=>!v)}><span className="highlightIcon" /></button>{highlightOpen && <div className="highlightPalette" onMouseDown={e=>e.preventDefault()}><button type="button" title="Yellow" aria-label="Yellow highlight" className="highlightSwatch yellow" onClick={()=>{changeHighlight('#ffff00');setHighlightOpen(false)}} /><button type="button" title="Green" aria-label="Green highlight" className="highlightSwatch green" onClick={()=>{changeHighlight('#00ff00');setHighlightOpen(false)}} /><button type="button" title="Red" aria-label="Red highlight" className="highlightSwatch red" onClick={()=>{changeHighlight('#ff0000');setHighlightOpen(false)}} /></div>}</div>}
    {multiline && <div className="sizeTool" title="Text size"><button type="button" onMouseDown={e=>e.preventDefault()} onClick={()=>changeSize(fontSize-1)}>−</button><span>{fontSize}</span><button type="button" onMouseDown={e=>e.preventDefault()} onClick={()=>changeSize(fontSize+1)}>+</button></div>}
    <select className="placeholderSelect" defaultValue="" aria-label="Insert field" onMouseDown={e=>e.stopPropagation()} onChange={e=>{if(e.target.value){addPlaceholder(e.target.value);e.currentTarget.value=''}}}><option value="">＋ Insert Field</option>{TEMPLATE_PLACEHOLDERS.map(key=><option key={key} value={key}>{key.replaceAll('_',' ')}</option>)}</select>
    {allowTable && multiline && <button type="button" className="toolbarActionButton" title="Insert AVCARD table" onMouseDown={e=>e.preventDefault()} onClick={addTable}>AVCARD Table</button>}
...
function Templates({ templates, setTemplates, customers, deals, notify }: { templates: Template[]; setTemplates: (t: Template[]) => void; customers: Customer[]; deals: Deal[]; notify: (s: string) => void }) {
  ...
  function openEditor(t?: Template) { setEditing(t || null); setName(t?.name || ''); setType(t?.type || 'Customer'); setCustomerIds(t?.customerIds || []); setSubject(t?.subject || (t?.type === 'FBO' ? fboSubject : customerSubject)); setBody(t?.body || (t?.type === 'FBO' ? fboBody : customerBody)); setOpen(true) }
  function close() { setOpen(false); setEditing(null); setName(''); setType('Customer'); setCustomerIds([]); setSubject(''); setBody('') }
  function save() { if (!name.trim() || !stripHtml(subject).trim() || !stripHtml(body).trim()) return notify('Template name, subject and body are required'); if (type === 'Customer' && customerIds.length === 0) return notify('Select at least one customer for a customer template'); const conflicts = type === 'Customer' ? customerIds.filter(id => assignedToOtherTemplate.has(id)) : []; if (conflicts.length) return notify('One or more selected customers are already assigned to another customer template'); const item: Template = { id: editing?.id || Date.now(), name: name.trim(), type, customerIds: type === 'Customer' ? customerIds : [], subject, body, updated: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }; setTemplates(editing ? templates.map(t => t.id === editing.id ? item : t) : [...templates, item]); notify(editing ? 'Template updated' : 'Template added'); close() }
  const assignedToOtherTemplate = new Set(templates.filter(t => t.type === 'Customer' && t.id !== editing?.id).flatMap(t => t.customerIds))
  const availableCustomers = customers.filter(c => customerIds.includes(c.id) || !assignedToOtherTemplate.has(c.id))
  const allCustomersSelected = availableCustomers.length > 0 && customerIds.length === availableCustomers.length
  const someCustomersSelected = customerIds.length > 0 && customerIds.length < availableCustomers.length
  useEffect(() => {
  ...
  function toggleCustomer(id: number) {
    if (!customerIds.includes(id) && assignedToOtherTemplate.has(id)) return
    setCustomerIds(ids => ids.includes(id) ? ids.filter(x => x !== id) : [...ids, id])
  }
  function selectAllCustomers() { setCustomerIds(availableCustomers.map(c => c.id)) }
...
function parseExcelBoolean(value: unknown, defaultValue = false) { const text=String(value??'').trim().toLowerCase(); if(['true','yes','y','1','active','included'].includes(text)) return true; if(['false','no','n','0','inactive','not included'].includes(text)) return false; return defaultValue }
...
function SettingsPage({ profile, setProfile, notify, customers, setCustomers, contacts, setContacts, aircraft, setAircraft, deals, setDeals, templates, setTemplates, schedule, setSchedule, generatedEmails, setGeneratedEmails, setSelectedEmail }: { profile:Profile; setProfile:(p:Profile)=>void; notify:(s:string)=>void; customers:Customer[]; setCustomers:(c:Customer[])=>void; contacts:Contact[]; setContacts:(c:Contact[])=>void; aircraft:Aircraft[]; setAircraft:(a:Aircraft[])=>void; deals:Deal[]; setDeals:(d:Deal[])=>void; templates:Template[]; setTemplates:(t:Template[])=>void; schedule:string; setSchedule:(s:string)=>void; generatedEmails:Email[]; setGeneratedEmails:(e:Email[])=>void; setSelectedEmail:(e:Email|null)=>void }) {
  ...
  function makeWorkbook(sample=false){
    ...
    appendSheet(wb,[['Corporate Fuel Management Tool — Excel Backup / Import Template'],['Use one worksheet for each type of information.'],['Customer Contacts supports multiple email addresses in one cell separated by semicolons.'],['Customer Templates use customer names separated by semicolons. FBO Templates use deal names separated by semicolons.'],['When importing, only worksheets with data rows replace existing data. Empty worksheets are ignored.']], 'Instructions')
    const customerRows: unknown[][]=[['ID','Customer Name','Customer Deals','Trip Number Included','Home Base Location']]; if(sample) customerRows.push(['1','Ahold','CAA Network Pricing','No','KTEB']); else customers.forEach(c=>customerRows.push([c.id,c.name,deals.filter(d=>c.dealIds.includes(d.id)).map(d=>d.name).join('; '),c.includeTripNumber?'Yes':'No',c.homeBase])); appendSheet(wb,customerRows,'Customers')
    const contactRows: unknown[][]=[['ID','Customer Name','Contact Name','Email Addresses']]; if(sample) contactRows.push(['1','Ahold','John Smith','ops@ahold.com; dispatch@ahold.com']); else contacts.forEach(c=>contactRows.push([c.id,customers.find(x=>x.id===c.customerId)?.name||'',c.name,c.emails.join('; ')])); appendSheet(wb,contactRows,'Customer Contacts')
    const aircraftRows: unknown[][]=[['ID','Tail','Aircraft Type','Customer Name','Home Base','AVCARD','Expiration','Active']]; if(sample) aircraftRows.push(['1','N123AB','Citation XLS','Ahold','KTEB','6010295001577742','07/2026','Yes']); else aircraft.forEach(a=>aircraftRows.push([a.id,a.tail,a.type,customers.find(c=>c.id===a.customerId)?.name||'',a.homeBase,a.avcard,a.expiration,a.active?'Yes':'No'])); appendSheet(wb,aircraftRows,'Aircraft')
    ...
    const scheduleRows: unknown[][]=[SCHEDULE_COLUMNS], parsedSchedule=parseSchedule(schedule).rows; if(sample) { scheduleRows.push(['27183','4/04','19:00','N838C','KRUQ','SALISBURY, NC','KJFK','NEW YORK, NY','75','GP / CM','4/04','20:27','1','Ahold','Andres','Salisbury Corporate Hangar','4/3/2026']); scheduleRows.push(['27183','4/04','22:35','N838C','KJFK','NEW YORK, NY','KOPF','MIAMI','86','GP / CM','4/05','00:13','2','Ahold','Andres','MODERN AVIATION','4/3/2026']); scheduleRows.push(['27183','4/05','10:35','N838C','KOPF','MIAMI','KRUQ','SALISBURY, NC','86','GP / CM','4/05','00:13','2','Ahold','Andres','FONTAINEBLEAU AVIATION','4/3/2026']); } else parsedSchedule.forEach(r=>scheduleRows.push(r)); appendSheet(wb,scheduleRows,'Schedule')
  ...
  async function importWorkbook(file:File){
    try{
      ...
      const getRows=(name:string)=>wb.Sheets[name]?XLSX.utils.sheet_to_json<Record<string,unknown>>(wb.Sheets[name],{defval:''}):[]
      const dealRows=getRows('Customer Deals'),customerRows=getRows('Customers'),contactRows=getRows('Customer Contacts'),aircraftRows=getRows('Aircraft'),templateRows=getRows('Templates'),emailRows=getRows('Generated Emails'),scheduleSheet=wb.Sheets['Schedule']
      if(![dealRows,customerRows,contactRows,aircraftRows,templateRows,emailRows,scheduleSheet?[1]:[]].some(rows=>rows.length)) return notify('No recognized CFM data sheets found')
      const importedDeals:Deal[]=dealRows.map((r,i)=>({id:numberId(r.ID,1000+i),name:String(r['Deal Name']||'').trim(),provider:String(r.Provider||'').trim(),description:String(r.Description||'').trim(),color:String(r.Color||'#1976e5').trim()||'#1976e5',tankering:parseExcelBoolean(r.Tankering,false)})).filter(d=>d.name&&d.provider)
      const importedCustomers:Customer[]=customerRows.map((r,i)=>({id:numberId(r.ID,2000+i),name:String(r['Customer Name']||'').trim(),dealIds:excelList(r['Customer Deals']).map(name=>importedDeals.find(d=>d.name.toLowerCase()===name.toLowerCase())?.id).filter((id):id is number=>typeof id==='number'),rule:'Customer + FBO emails',includeTripNumber:parseExcelBoolean(r['Trip Number Included'],false),homeBase:String(r['Home Base Location']||'').trim().toUpperCase()})).filter(c=>c.name)
      const customerByName=new Map(importedCustomers.map(c=>[c.name.toLowerCase(),c.id]))
      ...
      notify('Excel data imported successfully')

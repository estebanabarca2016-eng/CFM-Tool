...
function Customers({ customers, setCustomers, deals, contacts, setContacts, notify }: { customers: Customer[]; setCustomers: (c: Customer[]) => void; deals: Deal[]; contacts: Contact[]; setContacts: (c: Contact[]) => void; notify: (s: string) => void }) {
  ...
  function openContact(c?: Contact) { setContactEditing(c||null); setContactName(c?.name||''); setContactCustomerId(c?.customerId??(customers[0]?.id||null)); setContactEmails(c?.emails?.length?[...c.emails]:['']); setContactEditorOpen(true) }
  function closeEditors() { setEditorOpen(false); setContactEditorOpen(false); setEditing(null); setContactEditing(null); setName(''); setDealIds([]); setRule('Customer + FBO emails'); setIncludeTripNumber(false); setHomeBase(''); setContactName(''); setContactCustomerId(customers[0]?.id||null); setContactEmails(['']) }
  ...
  function saveContact() { if(!contactName.trim()) return notify('Contact name is required'); if(!contactCustomerId) return notify('Select a customer for this contact'); const emailsForContact=Array.from(new Set(contactEmails.map(e=>e.trim().toLowerCase()).filter(Boolean))); if(!emailsForContact.length) return notify('Enter at least one email address'); if(emailsForContact.some(e=>!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e))) return notify('Enter valid email addresses'); const item:Contact={id:contactEditing?.id||Date.now(),customerId:contactCustomerId,name:contactName.trim(),emails:emailsForContact}; setContacts(contactEditing?contacts.map(c=>c.id===contactEditing.id?item:c):[...contacts,item]); notify(contactEditing?'Contact updated':'Contact created'); closeEditors() }
  function addEmailField() { setContactEmails(emails=>[...emails,'']) }
  function removeEmailField(index:number) { setContactEmails(emails=>emails.length===1?['']:emails.filter((_,i)=>i!==index)) }
  return <div className="panel full">
    ...
    {contactEditorOpen&&<div className="editor"><h3>{contactEditing?'Edit Contact':'New Contact'}</h3><div className="formGrid"><label>Contact name<input value={contactName} onChange={e=>setContactName(e.target.value)} placeholder="John Smith"/></label><label>Customer<select value={contactCustomerId||''} onChange={e=>setContactCustomerId(e.target.value?Number(e.target.value):null)}>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><div className="wide"><span className="labelTitle">Email addresses</span><div className="contactEmailFields">{contactEmails.map((email,index)=><div className="contactEmailRow" key={index}><input type="email" value={email} onChange={e=>setContactEmails(values=>values.map((v,i)=>i===index?e.target.value:v))} placeholder={index===0?'name@company.com':'another@company.com'}/>{index===contactEmails.length-1?<button type="button" className="addEmailButton" title="Add another email" onClick={addEmailField}><Plus size={15}/></button>:<button type="button" className="removeEmailButton" title="Remove email" onClick={()=>removeEmailField(index)}><X size={14}/></button>}</div>)}</div><small className="fieldHint">Use the blue + button to add another email address.</small></div></div><div className="formActions"><button className="secondary" onClick={closeEditors}>Cancel</button><button className="primary" onClick={saveContact}><Check size={15}/>Save Contact</button></div></div>}
  </div>
}
...
function SettingsPage({ notify, customers, setCustomers, contacts, setContacts, aircraft, setAircraft, deals, setDeals, templates, setTemplates, schedule, setSchedule, generatedEmails, setGeneratedEmails, setSelectedEmail }: { notify:(s:string)=>void; customers:Customer[]; setCustomers:(c:Customer[])=>void; contacts:Contact[]; setContacts:(c:Contact[])=>void; aircraft:Aircraft[]; setAircraft:(a:Aircraft[])=>void; deals:Deal[]; setDeals:(d:Deal[])=>void; templates:Template[]; setTemplates:(t:Template[])=>void; schedule:string; setSchedule:(s:string)=>void; generatedEmails:Email[]; setGeneratedEmails:(e:Email[])=>void; setSelectedEmail:(e:Email|null)=>void }) {
  ...
  function makeWorkbook(sample=false){
    ...
    const customerRows=[['ID','Customer Name','Customer Deals','Trip Number Included','Home Base Location']]; if(sample) customerRows.push(['1','Ahold','CAA Network Pricing','No','KTEB']); else customers.forEach(c=>customerRows.push([c.id,c.name,deals.filter(d=>c.dealIds.includes(d.id)).map(d=>d.name).join('; '),c.includeTripNumber?'Yes':'No',c.homeBase])); appendSheet(wb,customerRows,'Customers')
    const contactRows=[['ID','Customer Name','Contact Name','Email Addresses']]; if(sample) contactRows.push(['1','Ahold','John Smith','ops@ahold.com; dispatch@ahold.com']); else contacts.forEach(c=>contactRows.push([c.id,customers.find(x=>x.id===c.customerId)?.name||'',c.name,c.emails.join('; ')])); appendSheet(wb,contactRows,'Customer Contacts')
    const aircraftRows=[['ID','Tail','Aircraft Type','Customer Name','Home Base','AVCARD','Expiration','Active']]; if(sample) aircraftRows.push(['1','N123AB','Citation XLS','Ahold','KTEB','6010295001577742','07/2026','Yes']); else aircraft.forEach(a=>aircraftRows.push([a.id,a.tail,a.type,customers.find(c=>c.id===a.customerId)?.name||'',a.homeBase,a.avcard,a.expiration,a.active?'Yes':'No'])); appendSheet(wb,aircraftRows,'Aircraft')
    const dealRows=[['ID','Deal Name','Provider','Description','Color','Tankering']]; if(sample) dealRows.push(['1','CAA Network Pricing','CAA','Corporate Aircraft Association network pricing program.','#1976e5','Yes']); else deals.forEach(d=>dealRows.push([d.id,d.name,d.provider,d.description,d.color,d.tankering?'Yes':'No'])); appendSheet(wb,dealRows,'Customer Deals')
    const templateRows=[['ID','Template Name','Type','Customers','Deals','Subject','Body','Updated']]; if(sample){templateRows.push(['1','Customer Schedule Example','Customer','Ahold','','Fuel Reservation – {{TAIL}} – {{ROUTE}} – {{DEPARTURE_DATE}}','<p>Hello {{CUSTOMER}},</p><p>Please review the fuel reservation for {{TAIL}}.</p>','Sep 10, 2026']); templateRows.push(['2','FBO Handling Example','FBO','','CAA Network Pricing','Handling Request – {{TAIL}} – {{ICAO}} – {{ARRIVAL_MONTH_DAY}}','<p>We have <b>{{TAIL}}</b> coming to <b>{{ARRIVAL_AIRPORT}}</b>/<b>{{ICAO}}</b>.</p>','Sep 10, 2026'])} else templates.forEach(t=>templateRows.push([t.id,t.name,t.type,t.customerIds.map(id=>customers.find(c=>c.id===id)?.name).filter(Boolean).join('; '),t.dealIds.map(id=>deals.find(d=>d.id===id)?.name).filter(Boolean).join('; '),t.subject,t.body,t.updated])); appendSheet(wb,templateRows,'Templates')
    const scheduleRows=[SCHEDULE_COLUMNS], parsedSchedule=parseSchedule(schedule).rows; if(sample) scheduleRows.push(['1024','09/17/2026','08:30','N123AB','KMIA','Miami','KTEB','Teterboro','02:45','3','09/17/2026','11:15','1','Ahold','John Smith','']); else parsedSchedule.forEach(r=>scheduleRows.push(r)); appendSheet(wb,scheduleRows,'Schedule')
    const emailRows=[['ID','Type','ICAO','IATA','FBO','Tail','Customer','To Emails','Subject','Body','Status']]; if(!sample) generatedEmails.forEach(e=>emailRows.push([e.id,e.type,e.icao,e.iata||'',e.fbo,e.tail,e.customer,(e.toEmails||[]).join('; '),e.subject,e.body,e.status])); appendSheet(wb,emailRows,'Generated Emails')
    return wb
  }

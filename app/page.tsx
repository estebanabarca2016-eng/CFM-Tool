...
type Page = 'Dashboard' | 'Schedule Processor' | 'Customers' | 'Aircraft' | 'Customer Deals' | 'Templates' | 'Airports' | 'Alerts' | 'Settings'
  ...
  return <div className="shell">
    ...
    <main className="main"><header><button className="hamb" onClick={() => setMobile(!mobile)}><Menu /></button><button className="iconBtn topAlertBtn" onClick={() => { setPage('Alerts'); notify('Opening alerts') }} aria-label="Alerts"><Bell size={19} /><em>{avcardAlerts.length}</em></button></header>
      <div className="content">{page !== 'Dashboard' && <div className="pageTitle"><div><h1>{title}</h1><p>{page === 'Schedule Processor' ? 'Paste your Excel schedule, review the communications that will be generated, and confirm.' : page === 'Customer Deals' ? 'Create the pricing deals and programs that can be assigned to customers.' : 'Manage aviation fuel operations, schedules and communications.'}</p></div></div>}
        ...
        {page === 'Customers' && <Customers customers={customers} setCustomers={setCustomers} deals={deals} contacts={contacts} setContacts={setContacts} notify={notify} />}
...
function Customers({ customers, setCustomers, deals, contacts, setContacts, notify }: { customers: Customer[]; setCustomers: (c: Customer[]) => void; deals: Deal[]; contacts: Contact[]; setContacts: (c: Contact[]) => void; notify: (s: string) => void }) {
  ...
  return <div className="panel full">
    <div className="sectionIntro"><div><h3>{activeTab}</h3><p>{activeTab==='Customers'?'Manage customers, pricing programs and communication rules.':'Manage customer contacts and all email addresses used for customer drafts.'}</p></div><button className="primary" onClick={()=>activeTab==='Customers'?openCustomer():openContact()}><Plus size={16}/>{activeTab==='Customers'?'Add Customer':'Add Contact'}</button></div>
    <div className="customerTabs"><button className={activeTab==='Customers'?'customerTab active':'customerTab'} onClick={()=>setActiveTab('Customers')}>Customers</button><button className={activeTab==='Contacts'?'customerTab active':'customerTab'} onClick={()=>setActiveTab('Contacts')}>Contacts</button></div>
...
function SettingsPage({ notify, customers, setCustomers, contacts, setContacts, aircraft, setAircraft, deals, setDeals, templates, setTemplates, schedule, setSchedule, generatedEmails, setGeneratedEmails, setSelectedEmail }: { notify:(s:string)=>void; customers:Customer[]; setCustomers:(c:Customer[])=>void; contacts:Contact[]; setContacts:(c:Contact[])=>void; aircraft:Aircraft[]; setAircraft:(a:Aircraft[])=>void; deals:Deal[]; setDeals:(d:Deal[])=>void; templates:Template[]; setTemplates:(t:Template[])=>void; schedule:string; setSchedule:(s:string)=>void; generatedEmails:Email[]; setGeneratedEmails:(e:Email[])=>void; setSelectedEmail:(e:Email|null)=>void }) {
  ...
  function makeWorkbook(sample=false){
    ...
    const templateRows: unknown[][]=[['ID','Template Name','Type','Customers','Deals','Subject','Body','Updated']]; if(sample){templateRows.push(['1','Customer Schedule Example','Customer','Ahold','','Fuel Reservation – {{TAIL}} – {{ROUTE}} – {{DEPARTURE_DATE}}','<p>Hello {{CUSTOMER}},</p><p>Please review the fuel reservation for {{TAIL}}.</p>','Sep 10, 2026']); templateRows.push(['2','FBO Handling Example','FBO','','CAA Network Pricing','Handling Request – {{TAIL}} – {{ICAO}} – {{ARRIVAL_MONTH_DAY}}','<p>We have <b>{{TAIL}}</b> coming to <b>{{ARRIVAL_AIRPORT}}</b>/<b>{{ICAO}}</b>.</p>','Sep 10, 2026'])} else templates.forEach(t=>templateRows.push([t.id,t.name,t.type,t.customerIds.map(id=>customers.find(c=>c.id===id)?.name).filter(Boolean).join('; '),t.dealIds.map(id=>deals.find(d=>d.id===id)?.name).filter(Boolean).join('; '),t.subject,t.body,t.updated])); appendSheet(wb,templateRows,'Templates')

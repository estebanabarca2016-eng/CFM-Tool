...
function AircraftPage({ aircraft, setAircraft, customers, notify }: { aircraft: Aircraft[]; setAircraft: (a: Aircraft[]) => void; customers: Customer[]; deals?: Deal[]; notify: (s: string) => void }) {
  ...
  function open(a?: Aircraft) { setEditorOpen(true); setEditing(a || null); setTail(a?.tail || ''); setType(a?.type || ''); setCustomerId(a?.customerId ?? null); setHomeBase(a?.homeBase || ''); setAvcard((a?.avcard || '').replace(/\D/g,'')); setExpiration(a?.expiration || '') }

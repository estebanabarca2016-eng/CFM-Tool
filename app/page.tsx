...
type Email = { id: number; type: 'FBO' | 'Customer'; icao: string; fbo: string; tail: string; customer: string; subject: string; body: string; status: 'Ready' | 'Pending' | 'Draft' }
...
function ScheduleProcessor({ schedule, setSchedule, notify, deals, customers, aircraft, templates, setGeneratedEmails, setPage }: { schedule: string; setSchedule: (s: string) => void; notify: (s: string) => void; deals: Deal[]; customers: Customer[]; aircraft: Aircraft[]; templates: Template[]; setGeneratedEmails: (e: Email[]) => void; setPage: (p: Page) => void }) {
  ...
  function confirm(){const customerEmail=communications.find(e=>e.type==='Customer');const fbos=communications.filter(e=>e.type==='FBO');if(!customerEmail&&!fbos.length)return notify('Select at least one FBO template or configure a customer template');setGeneratedEmails([...(customerEmail?[customerEmail]:[]),...fbos]);setPage('Dashboard');notify(customerEmail?'Emails generated and sent to Dashboard':'FBO emails generated and sent to Dashboard')}
